...
function ResultsPage() {
  ...
  useEffect(() => {
    // Fetch action steps for Awareness and Readiness assessments
    if ((assessmentType === 'Awareness' || assessmentType === 'Readiness') && assessment && !actionSteps) {
      let industry;
      if (assessmentType === 'Awareness') {
        industry = assessment?.awareness_info?.industry || user?.industry;
      } else if (assessmentType === 'Readiness') {
          ...
          <div className="flex items-start">
            ...
            <div className="flex flex-col items-center justify-center" style={{ width: '15%' }}>
              <MaturityStackedColumn 
                ...
                sectorAverage={(assessmentType === 'Awareness' || assessmentType === 'System' || assessmentType === 'Readiness' || assessmentType === 'Orgwide') ? sectorAverage : null}
                sectorName={
                    ...
                    The results indicate that <strong>{assessment?.orgwide_info?.org_name || user?.organization_name}</strong> has achieved an overall Organisation-wide AI Maturity score of <strong>{summary.overall_percentage.toFixed(1)}%</strong>, placing the organisation within the <strong>{summary.overall_maturity}</strong> maturity category{
                      sectorAverage !== null && benchmarkSector ? (
                        (() => {
                          ...
                          const comparison = userScore > avgScore ? 'above' : userScore < avgScore ? 'below' : 'equal to';
                          return (
                            <>, which is <strong>{comparison}</strong> the <strong>{benchmarkSector}</strong> sector AI maturity average of <strong>{avgScore}%</strong>.</>
                          );
                        })()
                      ) : '.'
                    } This rating reflects {
          ...
          <div className="p-4">
            <div className={viewMode === 'radar' ? '-mb-6' : 'mb-4'}>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {viewMode === 'radar' && (
                    <div className="text-[12px] text-gray-600 mt-1 ml-7">
                      <p>Comparing your AI domain {assessmentType === 'Awareness' ? 'awareness' : assessmentType === 'Readiness' ? 'readiness' : assessmentType === 'Orgwide' ? 'maturity' : 'maturity'} against the <span className="font-bold">{benchmarkSector || assessment?.system_info?.industry || assessment?.awareness_info?.industry || assessment?.readiness_info?.industry || assessment?.orgwide_info?.industry || user?.industry || 'sector'}</span> sector average.</p>
                ...
                {/* Radio buttons - show for System, Awareness, Readiness, and Orgwide assessments */}
                {(assessmentType === 'System' || assessmentType === 'Awareness' || assessmentType === 'Readiness' || assessmentType === 'Orgwide') && (
