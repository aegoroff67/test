...
async def generate_reports_background(assessment_id: str, assessment_type: str, user_id: str, org_id: str):
    """Generate PDF and DOCX reports in the background when an assessment is completed."""
    import traceback
    from report_generator import AMReportGenerator
    
    logger.info(f"Starting background report generation for assessment {assessment_id}")
    
    # Create or update report status record
    report_status_id = f"report_{assessment_id}"
    initial_status = {
        "id": report_status_id,
        "assessment_id": assessment_id,
        "assessment_type": assessment_type,
        "pdf_status": "generating",
        "docx_status": "generating",
        "pdf_error": None,
        "docx_error": None,
        "pdf_generated_at": None,
        "docx_generated_at": None,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
    
    await db.report_status.update_one(
        {"id": report_status_id},
        {"$set": initial_status},
        upsert=True
    )
    
    # Create a mock user object for report generation
    class MockUser:
        def __init__(self):
            self.id = user_id
            self.org_id = org_id
            self.email = "background@system"
            self.name = "Background Generator"
            self.organization_name = "System"
            self.role = "SUPER_ADMIN"  # Allow access to any assessment
    
    mock_user = MockUser()
    
    # Generate DOCX report
    try:
        logger.info(f"Generating DOCX report for assessment {assessment_id}")
        ...
        await db.generated_reports.update_one(
            {"assessment_id": assessment_id, "report_type": "docx"},
            {"$set": {
                "assessment_id": assessment_id,
                "report_type": "docx",
                "filename": filename,
        ...
        await db.report_status.update_one(
            ...
            {"$set": {
                "docx_status": "ready",
                "docx_generated_at": datetime.now(timezone.utc).isoformat(),
                "updated_at": datetime.now(timezone.utc).isoformat()
            }}
        )
        logger.info(f"DOCX report generated successfully for assessment {assessment_id}")
        
    except Exception as e:
        error_msg = str(e)[:500]
        logger.error(f"Error generating DOCX report for {assessment_id}: {error_msg}")
        await db.report_status.update_one(
            {"id": report_status_id},
            {"$set": {
                "docx_status": "failed",
                "docx_error": error_msg,
                "updated_at": datetime.now(timezone.utc).isoformat()
            }}
        )
    ...
    try:
        ...
        pdf_url = f"{frontend_url}/assessment/{assessment_id}/faira-results?print=true"
        ...
        pdf_filename = f"FAIRA_Results_{assessment_id}_{datetime.now().strftime('%Y%m%d')}.pdf"
        await db.generated_reports.update_one(
            {"assessment_id": assessment_id, "report_type": "pdf"},
            {"$set": {
                "assessment_id": assessment_id,
                "report_type": "pdf",
        ...
        logger.info(f"PDF report generated successfully for assessment {assessment_id}")
        
    except Exception as e:
        error_msg = str(e)[:500]
        logger.error(f"Error generating PDF report for {assessment_id}: {error_msg}")
        await db.report_status.update_one(
            {"id": report_status_id},
            {"$set": {
                "pdf_status": "failed",
...
async def login(user_data: UserLogin):
    ...
    user_response = UserResponse(
        ...
        industry=org.get("industry", "Unknown") if org else "Unknown",
        default_industry=user.get("default_industry"),
        tier=user.get("tier", 1),
        assessment_access=user.get("assessment_access", ["awareness"])
    )
...
@api_router.get("/admin/debug/assessment-schema")
async def debug_assessment_schema(admin: UserResponse = Depends(require_super_admin)):
    """Debug endpoint to check assessment document structure"""
    try:
        # Get a sample of assessments to check field structure
        samples = []
...
async def delete_assessment(
    ...
    # SUPER_ADMIN can delete any assessment, regular users can only delete their org's assessments
    if current_user.role == "SUPER_ADMIN":
        assessment = await db.assessments.find_one({"id": assessment_id})
    else:
...
async def update_faira_form(
    ...
    if status == "completed":
        ...
        import asyncio
        asyncio.create_task(generate_reports_background(
            assessment_id=assessment_id,
            assessment_type="Faira",
            user_id=current_user.id,
            org_id=current_user.org_id
...
@api_router.get("/assessments/{assessment_id}/report-status")
async def get_report_status(
    assessment_id: str,
    current_user: UserResponse = Depends(get_current_user)
):
    """Get the status of background report generation for an assessment."""
    report_status_id = f"report_{assessment_id}"
    status = await db.report_status.find_one({"id": report_status_id}, {"_id": 0})
    ...
    return {
        "assessment_id": assessment_id,
        "pdf_status": status.get("pdf_status", "not_started"),
        "docx_status": status.get("docx_status", "not_started"),
        "pdf_error": status.get("pdf_error"),
        "docx_error": status.get("docx_error"),
        "pdf_generated_at": status.get("pdf_generated_at"),
        "docx_generated_at": status.get("docx_generated_at")
    }


@api_router.get("/assessments/{assessment_id}/download-generated-report/{report_type}")
async def download_generated_report(
    assessment_id: str,
    report_type: str,
    current_user: UserResponse = Depends(get_current_user)
):
    """Download a pre-generated report (PDF or DOCX)."""
    from fastapi.responses import Response
    
    if report_type not in ["pdf", "docx"]:
        raise HTTPException(status_code=400, detail="Invalid report type. Use 'pdf' or 'docx'.")
    ...
    if not report:
        raise HTTPException(status_code=404, detail=f"Generated {report_type.upper()} report not found. Please wait for generation to complete.")
...
@api_router.post("/assessments/{assessment_id}/trigger-report-generation")
async def trigger_report_generation(
    assessment_id: str,
    current_user: UserResponse = Depends(get_current_user)
):
    """Manually trigger report generation for a completed assessment."""
    import asyncio
    ...
    if assessment_type.upper() == "FAIRA":
        assessment_type = "Faira"
...
@api_router.get("/system/action-steps/{sector:path}")
async def get_system_action_steps(sector: str):
    """
    Get AI system maturity action steps for a specific sector.
    Returns question codes mapped to sector-specific action steps.
    ...
    # Load system action steps from JSON file
    action_steps_path = os.path.join(os.path.dirname(__file__), "system_actions.json")
    try:
        with open(action_steps_path, 'r') as f:
            all_action_steps = json.load(f)
    except FileNotFoundError:
        raise HTTPException(
            status_code=500,
            detail="System action steps data file not found"
        )
    ...
    if not sector_actions:
        raise HTTPException(
            status_code=404,
            detail=f"No system action steps found for sector: {sector}"
        )
...
async def generate_faira_results_pdf(
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Expand only the specified control cards for the PDF
            try:
                ...
                logger.info(f"Found {len(control_cards)} control cards, expanding indices: {expanded_indices}")
        ...
        await log_audit_event(
            ...
            details={
                "report_type": "pdf",
                "pdf_type": "faira_results_summary",
                "assessment_type": "FAIRA",
                "assessment_name": assessment.get("name", ""),
                "file_size_bytes": pdf_file_size,
                "expanded_controls": list(expanded_indices)
            }
        )
        ...
        await log_analytics_event(
            ...
            properties={
                ...
                "pdf_type": "faira_results_summary",
                "file_size_bytes": pdf_file_size,
                "expanded_controls_count": len(expanded_indices)
            }
        )
...
async def generate_framework_coverage_pdf(
    ...
    """Generate Framework Coverage Overview PDF"""
    try:
        from playwright.async_api import async_playwright
        
        logger.info(f"Generating Framework Coverage PDF for assessment: {assessment_id}")
        ...
        logger.info(f"Generating PDF for URL: {coverage_url}")
        ...
        async with async_playwright() as p:
            ...
            logger.info(f"Auth token set, navigating to {coverage_url}")
            
            # Navigate to framework coverage page
            response = await page.goto(coverage_url, wait_until='networkidle', timeout=60000)
            logger.info(f"Page loaded with status: {response.status}")
            ...
            logger.info(f"Framework Coverage PDF generated at: {temp_pdf_path}")
            await browser.close()
        ...
        logger.error(f"Error generating framework coverage PDF: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to generate PDF: {str(e)}")
