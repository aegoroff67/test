...
class AMReportGenerator:
    ...
    def _get_template_for_assessment_type(self, assessment_type: str) -> str:
        ...
        template_map = {
            ...
            'FAIRA': 'AM_AI_SAFE_FAIRA_Report_TEMPLATE_v0.53_20260204_FIXED.docx',
        }
...
Each cell represents the score for a specific question, enabling identification of precise areas requiring attention and those demonstrating strong performance."""
            ...
            # Add FAIRA-specific variables if this is a FAIRA assessment
                ...
                try:
                    ...
                    if faira_domain_scores:
                        ...
                        template_context['domain_radar_risk'] = InlineImage(doc, io.BytesIO(domain_risk_radar_bytes), width=Inches(3.5))
                        ...
                        template_context['inherent_risk_radar'] = InlineImage(doc, io.BytesIO(inherent_risk_radar_bytes), width=Inches(3.5))
    ...
    def _load_template_with_ampersand_protection(self, template_path: str) -> DocxTemplate:
        ...
        Version: 2026-02-01-v1 - Uses in-memory processing to avoid temp file issues
        """
        ...
        # Pre-process the template to fix split Jinja tags (in-memory)
        try:
            fixed_template_bytes = self._fix_split_jinja_tags_in_memory(template_path)
            if fixed_template_bytes:
                print(f"DEBUG: Using in-memory pre-processed template")
                doc = DocxTemplate(io.BytesIO(fixed_template_bytes))
            else:
                print(f"DEBUG: No fixes needed, loading template directly")
    ...
    def _fix_split_jinja_tags_in_memory(self, template_path: str) -> bytes:
        """
        ...
        # Create the fixed template in memory
        output_buffer = io.BytesIO()
